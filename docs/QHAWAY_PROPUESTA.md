...
**Fase 2 — Backend y cubo OLAP (mes 6-12, EN MARCHA).** Esta fase **dejó de ser una promesa**: ya hay un backend **FastAPI + PostgreSQL** operativo en producción, servido por HTTPS (Let's Encrypt) en **`https://qhaway.tunky.net`**, con seguridad por CORS, límite de tasa y solo lectura, y la base de datos no expuesta a internet[^road-api]. El dashboard de la Fase 1 **ya consume esa API** (con respaldo automático a los datos estáticos si el servidor se cae), y la base ya sirve la serie nacional de 22 años, el histórico regional y el detalle distrital, **verificados contra cifras oficiales** (PIM 2025 ≈ S/ 272.5 mil M; Lima ≈ S/ 158.6 mil M; 1,892 distritos). Sobre esta base se completa el **Explorador Multidimensional** (§8.8) como cubo OLAP con cruces arbitrarios en vivo —el endpoint `/api/cubo` ya existe— con *drill-down* y granularidad mensual histórica sobre ~11.4 M de filas/año; se añaden la documentación OpenAPI, los módulos 2 (Cambio Climático) y 5/IPT consolidados, y la capa de IA sobre la API propia. La ingesta de los **CSV anuales completos del MEF** a PostgreSQL (2012-2026, detalle distrital, ~11 millones de filas por año) está en curso: el ejercicio **2024 ya se cargó desde el CSV íntegro** (11,191,489 filas → 1,891 distritos, PIM nacional S/ 262.1 mil M) y el resto se descarga en paralelo mediante un orquestador propio que baja varios años a la vez —el portal del MEF limita la velocidad por conexión, así que la descarga concurrente multiplica el rendimiento ≈ 4.8× y reduce la migración de días a horas—, con un *job* mensual automático de actualización. El esquema estrella se detalla en §7.7.
...
**Control de calidad — conciliación de datos.** Toda cifra del observatorio pasa por una **prueba de conciliación**: el presupuesto nacional de cada año es un único número, de modo que cada corte (por función, por sector, por nivel de gobierno, por distrito-ejecutora, por destino-META y por región) debe sumar exactamente ese total. La verificación se ejecutó sobre los 22 ejercicios y **todos los cortes cuadran al céntimo** con el total nacional (p. ej. PIM 2024 = S/ 262.1 mil M y 2025 = S/ 272.5 mil M, idénticos en los seis cortes; diferencia 0.00 en los 22 años). Este chequeo además detectó —y corrigió— un rasgo estructural del SIAF que de otro modo habría sesgado las cifras: **las municipalidades (gobiernos locales) no llevan "sector" en la fuente**, por lo que un agregado ingenuo por sector omitía ~S/ 52 mil M de gasto local; QHAWAY los reincorpora como categoría "Gobiernos Locales". Esta trazabilidad —cada total reconstruible y reconciliado contra la fuente— es justamente el estándar de rigor que un observatorio académico debe garantizar.
...
| 2 | Mes 6-12 | ✅ En marcha (backend live) | Backend FastAPI + PostgreSQL en HTTPS (`qhaway.tunky.net`), consumido por el dashboard; serie/regional/distrital ya servidos desde la BD; endpoint de cubo OLAP; ingesta CSV 2012-2026 en curso + job mensual | Cubo OLAP completo con cruces arbitrarios; distrital histórico cargado; API pública documentada con ≥1 caso de uso externo |
